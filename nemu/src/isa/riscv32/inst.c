...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & 0xFFFFFFFC);
...
//  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem	   , R, R(rd) = (src2 != 0 ? ((int32_t)src1 - (int32_t)src1 / (int32_t)src2 * (int32_t)src2): (int32_t)src1));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div	   , R, R(rd) = (src2 == 0 ? (int32_t)-1: 
																		((src1 == 0x80000000 && src2 == -1) ? src1:
  ...
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = (src2 != 0 ? ((uint32_t)src1 % (uint32_t)src2): (uint32_t)src1));
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (src2 == 0 ? src1:
																		((int32_t)src1 == 0x80000000 && (int32_t)src2 == -1) ? 0:
																		(int32_t)src1 % (int32_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2 != 0 ? ((uint32_t)src1 / (uint32_t)src2): (uint32_t)-1));

  //RV32 Zicsr
  int rs1 = BITS(s->isa.inst, 19, 15);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = CSR(imm & 0xfff); CSR(imm & 0xfff) = src1;);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm & 0xfff); CSR(imm & 0xfff) |= src1;);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd) = CSR(imm & 0xfff); CSR(imm & 0xfff) &= ~src1;);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, R(rd) = CSR(imm & 0xfff); CSR(imm & 0xfff) = (rs1 & 0x1f));
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(rd) = CSR(imm & 0xfff); CSR(imm & 0xfff) |= (rs1 & 0x1f));
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(rd) = CSR(imm & 0xfff); CSR(imm & 0xfff) &= ~(rs1 & 0x1f));
